...
def run(
    ...
    try:
        ...
        if not binary_path.exists():
            console.print(f"❌ [bold red]Error:[/bold red] Binary not found: {binary_path}")
            raise typer.Exit(1)
        
        if not binary_path.is_file():
            console.print(f"❌ [bold red]Error:[/bold red] Path is not a file: {binary_path}")
            raise typer.Exit(1)
...
def discover(
    ...
    try:
        ...
        console.print(f"🔍 [bold blue]Test Discovery[/bold blue]")
        console.print(f"   Binary: [cyan]{binary_path}[/cyan]")
        console.print()
...
def _display_discovered_tests(suites):
    ...
    for suite_name, suite in suites.items():
        ...
        suite_branch = tree.add(f"📁 [cyan]{suite_name}[/cyan] ({len(suite.cases)} tests)")
