...
from deflake.cli import app, _display_discovered_tests
...
class TestCLI:
    ...
    def test_discover_command_success(self, _mock_is_file, _mock_exists, mock_display, mock_discovery):
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=True)
    def test_run_command_no_flaky_behavior(self, mock_is_file, mock_exists, mock_runner_class, mock_menu_class, mock_discovery):
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=True)
    def test_run_command_flaky_behavior_detected(self, mock_is_file, mock_exists, mock_runner_class, mock_menu_class, mock_discovery):
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=True)
    def test_run_command_user_cancellation(self, mock_is_file, mock_exists, mock_menu_class, mock_discovery):
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=True)
    def test_run_command_custom_options(self, _mock_is_file, _mock_exists, mock_runner_class, mock_menu_class, mock_discovery):
        """Test run command with custom options."""
        # Setup minimal mocks to avoid actual execution
        mock_discovery.return_value.discover_tests.return_value = {"Suite": self.test_suite}
        ...
        suites = {
            "BasicTests": GTestSuite(
                ...
                cases=[
                    ...
                    GTestCase("Test2", "BasicTests.Test2", "BasicTests", is_parameterized=True)
                ]
            ),
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=True)
    def test_duration_conversion(self, _mock_is_file, _mock_exists, mock_runner_class, mock_menu_class, mock_discovery):
        ...
        result = self.runner.invoke(app, ["run", "/path/to/binary", "--duration", "120"])
